...
class Graph():
    ...
    def __exist_node__(self, i: Node):
        return self.__exist_node__(str(i))

    def __exist_node__(self, i: str):
        if str(i) not in self.nodes:
    ...
    def remove_edge(self, i: str, j: str):
        self.__exist_node__(i)
        self.__exist_node__(j)
        self.nodes[str(i)].remove_child(j)
        self.nodes[str(j)].remove_parent(i)

    def has_edge(self, i: str, j: str):
        self.__exist_node__(i)
        self.__exist_node__(j)
        return self.nodes[str(i)].has_child(j)

    def out_edges(self, i: str):
        return self.nodes[str(i)].child_nodes

    def in_edges(self, i: str):
        return self.nodes[str(i)].parent_nodes

    def to_dot(self):
        base = "digraph G {\nrankdir=LR;\nnode [shape = none]; \"\";\n"
        sout = ""
        sops = "node [shape = square];"
        svar = "node [shape = doublecircle];"
        spla = "node [shape = circle];"
        for n in self.nodes:
            r=type(self.nodes[n]).__name__
            if r=="Placeholder":
                spla +=" {}".format(str(n))
            elif r == "Variable":
                svar +=" {}".format(str(n))
                ...
                sops += " {}".format(str(n))
            for c in self.nodes[str(n)].child_nodes:
                sout += str(n) + "->" + str(c) + "\n"

        sout += str(n) + "->\"\" [ label=\"result\"]; "  +"\n}"
        return base + sops+ ";\n"+ svar+ ";\n"+ spla+ ";\n"+ sout
    ...
    def bfs(self, i: str):
        ...
        while len(Q) > 0:
            v = Q.popleft()
            visited.append(self.nodes[str(v)])
            for w in self.nodes[str(v)].parent_nodes:
                if w not in visited:
                    Q.append(w)
        return visited  # devolvemos la lista de los nodos en el orden "profundo" correcto
